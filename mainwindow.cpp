...
void MainWindow::init(const QString &profile, const QString& openFileName)
{
    ...
    this->ui->splitter->setSizes(QList<int>() << MainWindow::getOption("sidePanelWidth").toInt() << 1);
    ...
    connect(this->ui->view, &PageViewWidget::updateHorizontalScrollBar, [this](int max, int current, int visible){
        ...
        this->scr_h->setVisible(this->ui->actionShow_scrollbars->isChecked() &&
                                (getOption("alwaysShowScrollbars").toBool() || max > 0));
    ...
    connect(this->ui->view, &PageViewWidget::updateVerticalScrollBar, [this](int max, int current, int visible){
        ...
        this->scr_v->setVisible(this->ui->actionShow_scrollbars->isChecked() &&
                                (getOption("alwaysShowScrollbars").toBool() || max > 0));
        scr_v->blockSignals(false);
    });
    connect(scr_v, &QScrollBar::valueChanged, this->ui->view, &PageViewWidget::setVerticalScrollPosition);
    connect(this->ui->fileSystemView, &QTreeView::doubleClicked, [this](const QModelIndex& index)
    {
    ...
    this->ui->bookmarksTreeWidget->setColumnWidth(0, MainWindow::getOption("sidePanelWidth").toInt()/3);
    this->ui->bookmarksTreeWidget->setColumnWidth(2, MainWindow::getOption("sidePanelWidth").toInt()/3);
    ...
    connect(this->ui->bookmarksTreeWidget, &QTreeWidget::itemChanged, [this](QTreeWidgetItem *item, int column)
    {
        this->saveBookmarksFromTreeWidget();
    });
    connect(this->ui->bookmarksTreeWidget, &QTreeWidget::itemDoubleClicked, [this](QTreeWidgetItem *item, int column)
    {
...
QColor MainWindow::getMostCommonEdgeColor(const QPixmap &left_pixmap, const QPixmap &right_pixmap)
{
    ...
    if(left_im_w > 0)
    {
        for(int i = 0; i < left_im_h; i++) colorCountMap[left_img.pixel(0, i)]++;
    }

    //Left image - top
    if(left_im_h > 0)
    {
        for(int i = 0; i < left_im_w; i++) colorCountMap[left_img.pixel(i, 0)]++;
    }

    //Left image - bottom
    if(left_im_h > 0)
    {
        for(int i = 0; i < left_im_w; i++) colorCountMap[left_img.pixel(i, left_im_h - 1)]++;
    }

    //Left image - right side
    if(left_im_w > 0 && left_img.isNull())
    {
        for(int i = 0; i < left_im_h; i++) colorCountMap[left_img.pixel(left_im_w - 1, i)]++;
    }
    ...
    if(!right_img.isNull())
    {
        ...
        if(right_im_h > 0)
        {
            for(int i = 0; i < right_im_w; i++) colorCountMap[right_img.pixel(i, 0)]++;
        }

        //Right image - bottom
        if(right_im_h > 0)
        {
            for(int i = 0; i < right_im_w; i++) colorCountMap[right_img.pixel(i, right_im_h - 1)]++;
        }

        //Right image - right side
        if(right_im_w > 0 && right_img.isNull())
        {
            for(int i = 0; i < right_im_h; i++) colorCountMap[right_img.pixel(right_im_w - 1, i)]++;
        }
    }
...
void MainWindow::rebuildOpenMenu(QAction *action, const QStringList &strList, bool image)
{
    QString sep = ":";
...
{
    auto res = QFileDialog::getOpenFileName(this, QString{}, QString{}, "Zip archives (*.zip *.cbz);;Any file (*.*)");
    if(!res.isEmpty())
    {
        this->loadComic(createComicSource(res));
    }
...
void MainWindow::on_actionGo_to_page_triggered()
{
    ...
    {
        bool ok = false;
        int page = QInputDialog::getInt(this, "Go to page", "Page:", ui->view->getCurrentPage(), 1, ui->view->comicSource()->getPageCount(), 1, &ok);
        if(ok)
        {
...
void MainWindow::updateUIState()
{
    ...
    this->ui->actionOpen_image_with->setEnabled(src && this->ui->view->getCurrentPage() > 0);
    this->ui->actionDouble_page_mode->setChecked(this->ui->view->getDoublePageMode());
    this->ui->actionManga_mode->setChecked(this->ui->view->getMangaMode());
    this->ui->actionStretch_small_images->setChecked(this->ui->view->getStretchSmallImages());
    ...
    this->ui->actionAdd_bookmark->setEnabled(src && this->ui->view->getCurrentPage() > 0);
    ...
    this->ui->actionFlip_vertically->setEnabled(src && this->ui->view->getCurrentPage() > 0);
    this->ui->actionFlip_horizontally->setEnabled(src && this->ui->view->getCurrentPage() > 0);
    this->ui->actionRotate_clockwise->setEnabled(src && this->ui->view->getCurrentPage() > 0);
    this->ui->actionRotate_counterclockwise->setEnabled(src && this->ui->view->getCurrentPage() > 0);
    this->ui->actionRotate_180_degrees->setEnabled(src && this->ui->view->getCurrentPage() > 0);
...
void MainWindow::saveBookmarksFromTreeWidget()
{
    ...
    for(int i = 0; i < itemCount; i++)
    {
        ...
        {
            fail = true;
            QMessageBox::critical(this, "Error", "Can't save bookmarks: invalid page number!");
            break;
        }
...
void MainWindow::on_actionAllow_free_drag_triggered(bool checked)
{
    ...
    {
        QMessageBox::warning(this, "Free drag", "Free drag is not compatible with some advanced scrolling features. Proceed at your own risk.");
    }
