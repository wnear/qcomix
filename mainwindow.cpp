    ...
    , ui(new Ui::MainWindow)
{
    ...
    static_cast<QGridLayout*>(this->ui->viewPage->layout())
        ->addWidget(scr_h, 1, 0);
    ...
    for (int i = 1; i < this->ui->fileSystemView->header()->count(); i++) {
        this->ui->fileSystemView->setColumnHidden(i, true);
    }
    this->ui->fileSystemView->setSelectionMode(
        QAbstractItemView::SingleSelection);
    this->ui->fileSystemView->header()->setSectionResizeMode(
        QHeaderView::ResizeToContents);
    this->ui->fileSystemView->header()->setStretchLastSection(false);
    this->ui->fileSystemView->setExpandsOnDoubleClick(false);
    this->ui->bookmarksTreeWidget->setItemDelegateForColumn(
        0, new NoEditDelegate(this));
    ...
    this->ui->bookmarksTreeWidget->setSelectionMode(
        QTreeWidget::ExtendedSelection);
    this->ui->bookmarksTreeWidget->installEventFilter(
        new BookmarksTreeWidgetEventFilter(this));
...
{
    auto userConfigLocation = QStandardPaths::locate(
        QStandardPaths::AppConfigLocation, profile + ".conf");
    ...
    this->ui->splitter->setSizes(
        QList<int>() << MainWindow::getOption("sidePanelWidth").toInt() << 1);

    this->ui->actionFit_original_size_mode->setIcon(
        QIcon(getIconFileName("fit-original-size")));
    this->ui->actionFit_to_fixed_size_mode->setIcon(
        QIcon(getIconFileName("fit-fixed-size")));
    this->ui->actionBest_fit_mode->setIcon(QIcon(getIconFileName("fit-best")));
    this->ui->actionManual_zoom_mode->setIcon(
        QIcon(getIconFileName("fit-manual-zoom")));
    this->ui->actionFit_width_mode->setIcon(QIcon(getIconFileName("fit-width")));
    this->ui->actionFit_height_mode->setIcon(
        QIcon(getIconFileName("fit-height")));
    this->ui->actionPrevious_comic->setIcon(
        QIcon(getIconFileName("prev-archive")));
    ...
    this->ui->mainToolBar->insertWidget(this->ui->actionBest_fit_mode,
        expandWidget);

    if (getOption("startMaximized").toBool())
        this->showMaximized();
    if (getOption("startFullscreen").toBool())
        this->ui->actionFullscreen->toggle();
    if (!getOption("showMenubar").toBool())
        this->ui->actionShow_menu->toggle();
    if (!getOption("showToolbar").toBool())
        this->ui->actionShow_main_toolbar->toggle();
    if (!getOption("showSidePanel").toBool())
        this->ui->actionShow_side_panels->toggle();
    ...
    this->ui->actionShow_scrollbars->setChecked(
        getOption("showScrollbars").toBool());

    if (getOption("quitOnEscape").toBool()) {
        QShortcut* esc = new QShortcut(Qt::Key_Escape, this);
        connect(esc, &QShortcut::activated, QApplication::instance(),
            &QApplication::quit);
        connect(esc, &QShortcut::activatedAmbiguously, QApplication::instance(),
            &QApplication::quit);
    }

    if (getOption("useCurrentPageAsWindowIcon").toBool()) {
        connect(this->ui->view, &PageViewWidget::windowIconUpdateNeeded, this,
            &MainWindow::updateWindowIcon);
    }
    connect(this->ui->view, &PageViewWidget::archiveMetadataUpdateNeeded, this,
        &MainWindow::updateArchiveMetadata);
    connect(this->ui->view, &PageViewWidget::fitModeChanged,
        [this](PageViewWidget::FitMode fitMode) {
            this->ui->actionManual_zoom_mode->setChecked(
                fitMode == PageViewWidget::FitMode::ManualZoom);
            this->ui->actionFit_width_mode->setChecked(
                fitMode == PageViewWidget::FitMode::FitWidth);
            this->ui->actionFit_height_mode->setChecked(
                fitMode == PageViewWidget::FitMode::FitHeight);
            this->ui->actionBest_fit_mode->setChecked(
                fitMode == PageViewWidget::FitMode::FitBest);
            this->ui->actionFit_original_size_mode->setChecked(
                fitMode == PageViewWidget::FitMode::OriginalSize);
            this->ui->actionFit_to_fixed_size_mode->setChecked(
                fitMode == PageViewWidget::FitMode::FixedSize);
        });
    connect(this->ui->mainToolBar, &QToolBar::visibilityChanged,
        [this](bool visible) {
            if (!visible) {
                this->ui->actionShow_main_toolbar->setChecked(false);
            }
        });
    connect(this->ui->view, &PageViewWidget::currentPageChanged,
        [this](const QString& filePath, int current, int max) {
    ...
    connect(this->ui->view, &PageViewWidget::requestLoadNextComic, this,
        &MainWindow::on_actionNext_comic_triggered);
    connect(this->ui->view, &PageViewWidget::requestLoadPrevComic, this,
        &MainWindow::on_actionPrevious_comic_triggered);
    connect(this->ui->view, &PageViewWidget::imageMetadataUpdateNeeded,
        [this](PageMetadata m1, PageMetadata m2) {
            ...
            this->ui->im1vFileSize->setText(
                this->locale().formattedDataSize(m1.fileSize));
            this->ui->im1vDimensions->setText(QString("%1x%2").arg(
                QString::number(m1.width), QString::number(m1.height)));

            this->ui->im2vMime->setText(m2.fileType);
            this->ui->im2vFileName->setText(m2.fileName);
            this->ui->im2vFileSize->setText(
                this->locale().formattedDataSize(m2.fileSize));
            this->ui->im2vDimensions->setText(QString("%1x%2").arg(
                QString::number(m2.width), QString::number(m2.height)));
        });

    connect(this->ui->view, &PageViewWidget::updateHorizontalScrollBar,
        [this](int max, int current, int visible) {
            ...
            this->scr_h->setVisible(
                this->ui->actionShow_scrollbars->isChecked() && (getOption("alwaysShowScrollbars").toBool() || max > 0));
            scr_h->blockSignals(false);
        });
    connect(scr_h, &QScrollBar::valueChanged, this->ui->view,
        &PageViewWidget::setHorizontalScrollPosition);

    connect(this->ui->view, &PageViewWidget::updateVerticalScrollBar,
        [this](int max, int current, int visible) {
            const static int arrowKeyScrollPixels = MainWindow::getOption("arrowKeyScrollPixels").toInt();
            ...
            this->scr_v->setVisible(
                this->ui->actionShow_scrollbars->isChecked() && (getOption("alwaysShowScrollbars").toBool() || max > 0));
            scr_v->blockSignals(false);
        });
    connect(scr_v, &QScrollBar::valueChanged, this->ui->view,
        &PageViewWidget::setVerticalScrollPosition);
    connect(this->ui->fileSystemView, &QTreeView::doubleClicked,
        [this](const QModelIndex& index) {
            auto filePath = this->fileSystemModel
                                .fileInfo(this->fileSystemFilterModel.mapToSource(index))
                                .absoluteFilePath();
            this->ui->fileSystemView->scrollTo(
                fileSystemFilterModel.mapToSource(index));
            if (auto comic = createComicSource(filePath)) {
                this->loadComic(comic);
            }
        });
    connect(this->ui->view, &PageViewWidget::pageViewConfigUINeedsToBeUpdated,
        this, &MainWindow::updateUIState);
    this->ui->bookmarksTreeWidget->setColumnWidth(
        0, MainWindow::getOption("sidePanelWidth").toInt() / 3);
    this->ui->bookmarksTreeWidget->setColumnWidth(
        2, MainWindow::getOption("sidePanelWidth").toInt() / 3);
    this->ui->bookmarksTreeWidget->setColumnWidth(1, 40);
    this->ui->bookmarksTreeWidget->header()->setSectionResizeMode(
        2, QHeaderView::Fixed);
    connect(this->ui->bookmarksTreeWidget, &QTreeWidget::itemChanged,
        [this](QTreeWidgetItem* item, int column) {
            this->saveBookmarksFromTreeWidget();
        });
    connect(this->ui->bookmarksTreeWidget, &QTreeWidget::itemDoubleClicked,
        [this](QTreeWidgetItem* item, int column) {
        ...
        if (!QFileInfo(filePath).exists()) {
            ...
            if (!QDir().mkpath(path)) {
                QMessageBox::critical(nullptr, "Error",
                    "Failed to create the remembered pages file!");
            }
            QFile f(filePath);
        ...
        if (f.open(QFile::ReadOnly)) {
            ...
            if (err.error == QJsonParseError::NoError) {
                ...
                QMessageBox::critical(
                    nullptr, "Error",
                    "The content of the remembered pages file is invalid!");
            }
            f.close();
        } else {
            QMessageBox::critical(
                nullptr, "Error",
                "Failed to load bookmarks, can't open the remembered pages file!");
        }
    }
...
void MainWindow::savePositionForFilePath(const QString& p, int page)
{
    ...
    if (f.open(QFile::WriteOnly)) {
        ...
        QMessageBox::critical(nullptr, "Error",
            "Failed to save remembered pages, can't open the "
            "remembered pages file!");
    }
}
...
QVariant MainWindow::getOption(const QString& key)
{
    ...
    QMessageBox::critical(
        nullptr, "Error",
        QString("Missing/invalid configuration key: %1").arg(key));
    qFatal("Invalid key: %s", qPrintable(key));
}

QColor MainWindow::getMostCommonEdgeColor(const QPixmap& left_pixmap,
    const QPixmap& right_pixmap)
{
    QMap<QRgb, int> colorCountMap;
    auto left_img = left_pixmap.toImage();
    auto right_img = right_pixmap.toImage();
    ...
    if (left_im_w > 0) {
        for (int i = 0; i < left_im_h; i++)
            colorCountMap[left_img.pixel(0, i)]++;
    }

    // Left image - top
    if (left_im_h > 0) {
        for (int i = 0; i < left_im_w; i++)
            colorCountMap[left_img.pixel(i, 0)]++;
    }

    // Left image - bottom
    if (left_im_h > 0) {
        for (int i = 0; i < left_im_w; i++)
            colorCountMap[left_img.pixel(i, left_im_h - 1)]++;
    }

    // Left image - right side
    if (left_im_w > 0 && left_img.isNull()) {
        for (int i = 0; i < left_im_h; i++)
            colorCountMap[left_img.pixel(left_im_w - 1, i)]++;
    }

    if (!right_img.isNull()) {
        // Right image - top
        if (right_im_h > 0) {
            for (int i = 0; i < right_im_w; i++)
                colorCountMap[right_img.pixel(i, 0)]++;
        }

        // Right image - bottom
        if (right_im_h > 0) {
            for (int i = 0; i < right_im_w; i++)
                colorCountMap[right_img.pixel(i, right_im_h - 1)]++;
        }

        // Right image - right side
        if (right_im_w > 0 && right_img.isNull()) {
            for (int i = 0; i < right_im_h; i++)
                colorCountMap[right_img.pixel(right_im_w - 1, i)]++;
        }
    }
...
void MainWindow::on_actionHide_all_toggled(bool arg1)
{
    if (arg1) {
        ...
        if (prevMenuVisible && !this->ui->actionShow_menu->isChecked())
            this->ui->actionShow_menu->toggle();
        if (prevToolBarVisible && !this->ui->actionShow_main_toolbar->isChecked())
            this->ui->actionShow_main_toolbar->toggle();
        if (prevSidePanelVisible && !this->ui->actionShow_side_panels->isChecked())
            this->ui->actionShow_side_panels->toggle();
        if (prevScrollBarsVisible && !this->ui->actionShow_scrollbars->isChecked())
            this->ui->actionShow_scrollbars->toggle();
    }
}
...
void MainWindow::on_actionShow_side_panels_toggled(bool arg1)
{
    ...
    if (this->ui->sidePanel->isVisible())
...
void MainWindow::on_actionShow_scrollbars_toggled(bool arg1)
{
    ...
    if (this->scr_h->isVisible() || this->scr_v->isVisible())
        this->ui->actionHide_all->setChecked(false);
}
...
void MainWindow::loadComic(ComicSource* src)
{
    ...
    if (src) {
        ...
        if (getOption("useFirstPageAsWindowIcon").toBool() && src->getPageCount() > 0) {
            setWindowIcon(src->getPage(0).scaled(256, 256, Qt::KeepAspectRatio));
        } else {
            setWindowIcon(QIcon(":/icon.png"));
        }

        const QModelIndex rootIndex = fileSystemModel.index(src->getFilePath());
        this->ui->fileSystemView->setCurrentIndex(
            fileSystemFilterModel.mapFromSource(rootIndex));
        this->ui->fileSystemView->setExpanded(
            fileSystemFilterModel.mapFromSource(rootIndex), false);
        this->ui->fileSystemView->scrollTo(
            fileSystemFilterModel.mapFromSource(rootIndex));
        ...
        this->ui->fileSystemView->setCurrentIndex(
            fileSystemFilterModel.mapFromSource(rootIndex));
        this->ui->fileSystemView->setExpanded(
            fileSystemFilterModel.mapFromSource(rootIndex), true);
        this->ui->fileSystemView->scrollTo(
            fileSystemFilterModel.mapFromSource(rootIndex));
...
void MainWindow::updateWindowTitle()
{
    ...
    if (currentPageInWindowTitle != 0 && maxPageInWindowTitle != 0 && MainWindow::getOption("useComicNameAsWindowTitle").toBool()) {
        title = QString("[%1/%2] ")
                    .arg(QString::number(currentPageInWindowTitle),
                        QString::number(maxPageInWindowTitle));
    }
...
void MainWindow::saveBookmarks(const QJsonArray& bookmarks)
{
    ...
    if (f.open(QFile::WriteOnly)) {
        ...
        QMessageBox::critical(
            this, "Error",
            "Failed to save bookmarks, can't open the bookmark file!");
    }
}
...
void MainWindow::loadBookmarks()
{
    ...
    if (!QFileInfo(filePath).exists()) {
        ...
        if (!QDir().mkpath(path)) {
            QMessageBox::critical(this, "Error",
                "Failed to create the bookmark file!");
        }
        QFile f(filePath);
        if (f.open(QFile::WriteOnly)) {
            f.write(doc.toJson());
    ...
    if (f.open(QFile::ReadOnly)) {
        ...
        if (err.error == QJsonParseError::NoError) {
            ...
            QMessageBox::critical(this, "Error",
                "The content of the bookmark file is invalid!");
        }
        f.close();
    } else {
        QMessageBox::critical(
            this, "Error",
            "Failed to load bookmarks, can't open the bookmark file!");
    }
}
...
void MainWindow::loadRecentFiles()
{
    if (MainWindow::getOption("storeRecentFiles").toBool()) {
        ...
        if (!QFileInfo(filePath).exists()) {
            ...
            if (!QDir().mkpath(path)) {
                QMessageBox::critical(
                    this, "Error", "Failed to create the recent files storage file!");
            }
            QFile f(filePath);
            if (f.open(QFile::WriteOnly)) {
                f.write(doc.toJson());
        ...
        if (f.open(QFile::ReadOnly)) {
            ...
            if (err.error == QJsonParseError::NoError) {
                ...
                QMessageBox::critical(
                    this, "Error",
                    "The content of the recent files storage file is invalid!");
            }
            f.close();
        } else {
            QMessageBox::critical(this, "Error",
                "Failed to load recent files, can't open the "
...
void MainWindow::addToRecentFiles(const QString& path)
{
    ...
    while (maxNum >= 0 && recentFiles.length() > maxNum)
...
void MainWindow::rebuildRecentFilesMenu()
{
    if (MainWindow::getOption("storeRecentFiles").toBool()) {
        ...
        if (!this->ui->actionRecent->menu())
            this->ui->actionRecent->setMenu(new QMenu {});
        ...
        for (const auto& f : this->recentFiles) {
            ...
            if (info.exists()) {
                ...
                openRecent->setText(info.fileName());
                connect(openRecent, &QAction::triggered,
                    [this, f]() { this->loadComic(createComicSource(f)); });
...
void MainWindow::saveRecentFiles()
{
    if (MainWindow::getOption("storeRecentFiles").toBool()) {
        ...
        if (f.open(QFile::WriteOnly)) {
            ...
            QMessageBox::critical(this, "Error",
                "Failed to save recent files, can't open the "
                "recent files storage file!");
        }
    }
}
...
QString MainWindow::readLastViewedFilePath()
{
    ...
    if (!QFileInfo(filePath).exists()) {
        if (!QDir().mkpath(path)) {
            QMessageBox::critical(
                this, "Error",
                "Failed to create the file storing the last viewed comic!");
        }
        QFile f(filePath);
...
void MainWindow::updateBookmarkSideBar(const QJsonArray& bookmarks)
{
    ...
    if (bookmarks.size() % 3 != 0) {
        QMessageBox::critical(this, "Error",
            "Can't load bookmarks, invalid bookmark array!");
    } else {
...
{
    this->rebuildOpenMenu(this->ui->actionOpen_image_with,
        MainWindow::getOption("openImageWith").toStringList(),
        true);
}
...
void MainWindow::rebuildOpenWithMenu(ComicSource* src)
{
    if (src) {
        if (dynamic_cast<DirectoryComicSource*>(src)) {
            this->rebuildOpenMenu(
                this->ui->actionOpen_with,
                MainWindow::getOption("openDirectoryWith").toStringList(), false);
        } else if (dynamic_cast<ZipComicSource*>(src)) {
            this->rebuildOpenMenu(this->ui->actionOpen_with,
                MainWindow::getOption("openZipWith").toStringList(),
                false);
        }
    } else {
    ...
    bool image)
{
    ...
    for (int i = 0; i < l; ++i) {
        ...
        if (!programName.isEmpty()) {
            ...
            connect(openImg, &QAction::triggered,
                [this, programName, commandList, image]() mutable {
                    auto l = commandList.size();
                    if (!l)
                        return;
                    QString fileName;
                    if (image) {
                        fileName = this->ui->view->getCurrentPageFilePath();
            ...
            programName.clear();
            if (i < l && strList[i].startsWith(sep))
                i--;
        }
    }
...
void MainWindow::closeEvent(QCloseEvent* event)
{
    if (getOption("confirmQuit").toBool()) {
        if (QMessageBox::question(this, "Quitting qcomix",
...
{
    if (auto iconPath = MainWindow::getOption("iconPath").toString();
        iconPath == "default") {
...
void MainWindow::deleteSelectedBookmarks()
{
    ...
    if (sel.size()) {
        ...
        if (sel.size() > 1)
            question = "Are you sure you want to delete the selected bookmarks?";
        if (QMessageBox::question(this,
                "Delete bookmark" + (sel.size() > 1 ? QString("s") : QString {}),
                question)
            == QMessageBox::Yes) {
            for (int i = 0; i < sel.size(); i++) {
                delete this->ui->bookmarksTreeWidget->takeTopLevelItem(
                    this->ui->bookmarksTreeWidget->indexOfTopLevelItem(sel[i]));
            }
            this->saveBookmarksFromTreeWidget();
        }
    }
...
void MainWindow::on_actionNext_page_triggered() { this->ui->view->nextPage(); }

void MainWindow::updateWindowIcon(QImage page)
{
    setWindowIcon(QPixmap::fromImage(page.scaled(256, 256, Qt::KeepAspectRatio)));
}
...
void MainWindow::on_actionClose_comic_triggered() { this->loadComic(nullptr); }

void MainWindow::on_actionOpen_directory_triggered()
{
    auto res = QFileDialog::getExistingDirectory(this);
    if (!res.isEmpty()) {
        this->loadComic(createComicSource(res));
    }
}
...
void MainWindow::on_actionReload_triggered()
{
    ...
    int currentPage = this->ui->view->getCurrentPage();
    if (src) {
...
void MainWindow::on_actionGo_to_page_triggered()
{
    if (ui->view->comicSource()) {
        ...
        int page = QInputDialog::getInt(
            this, "Go to page", "Page:", ui->view->getCurrentPage(), 1,
            ui->view->comicSource()->getPageCount(), 1, &ok);
        if (ok) {
            ui->view->goToPage(page);
        }
    }
}
...
{
    auto fileInfo = static_cast<QFileSystemModel*>(this->sourceModel())
                        ->fileInfo(static_cast<QFileSystemModel*>(this->sourceModel())
...
void MainWindow::on_actionLast_page_triggered() { this->ui->view->lastPage(); }
...
void MainWindow::on_actionCopy_image_file_to_clipboard_triggered()
{
    ...
    auto path = this->ui->view->getCurrentPageFilePath();
    if (!path.isEmpty()) {
...
void MainWindow::on_actionZoom_in_triggered() { this->ui->view->zoomIn(); }

void MainWindow::on_actionZoom_out_triggered() { this->ui->view->zoomOut(); }
...
void MainWindow::on_actionSet_slideshow_interval_triggered()
{
    if (ui->view->comicSource()) {
        ...
        int seconds = QInputDialog::getInt(
            ...
            1, 900, 1, &ok);
        if (ok) {
            ui->view->setSlideShowSeconds(seconds);
        }
    }
}
...
void MainWindow::updateUIState()
{
    ...
    this->ui->actionPrevious_page->setEnabled(
        src && (!this->ui->view->onFirstPage() || (autoOpenNextComic && src->hasPreviousComic())));
    this->ui->actionNext_page->setEnabled(
        src && (!this->ui->view->onLastPage() || (autoOpenNextComic && src->hasNextComic())));
    this->ui->actionGo_to_page->setEnabled(src && src->getPageCount() > 0);
    this->ui->actionOpen_image_with->setEnabled(
        src && this->ui->view->getCurrentPage() > 0);
    this->ui->actionDouble_page_mode->setChecked(
        this->ui->view->getDoublePageMode());
    this->ui->actionManga_mode->setChecked(this->ui->view->getMangaMode());
    this->ui->actionStretch_small_images->setChecked(
        this->ui->view->getStretchSmallImages());
    this->ui->actionSmart_scroll_vertical_first->setChecked(
        this->ui->view->smartScrollVerticalFirst());
    this->ui->actionKeep_transformation->setChecked(
        this->ui->view->transformationKept());
    this->ui->actionSmart_scroll_vertical_first->setEnabled(
        this->ui->view->smartScrollEnabled());
    this->ui->actionFlip_horizontally->setChecked(
        this->ui->view->horizontallyFlipped());
    this->ui->actionFlip_vertically->setChecked(
        this->ui->view->verticallyFlipped());
    this->ui->actionReload->setEnabled(src);
    this->ui->actionClose_comic->setEnabled(src);
    this->ui->actionAdd_bookmark->setEnabled(
        src && this->ui->view->getCurrentPage() > 0);
    this->ui->actionReset_zoom->setEnabled(src && this->ui->view->getCurrentPage() > 0);
    this->ui->actionZoom_in->setEnabled(src && this->ui->view->getCurrentPage() > 0);
    this->ui->actionZoom_out->setEnabled(src && this->ui->view->getCurrentPage() > 0);
    this->ui->actionFlip_vertically->setEnabled(
        src && this->ui->view->getCurrentPage() > 0);
    this->ui->actionFlip_horizontally->setEnabled(
        src && this->ui->view->getCurrentPage() > 0);
    this->ui->actionRotate_clockwise->setEnabled(
        src && this->ui->view->getCurrentPage() > 0);
    this->ui->actionRotate_counterclockwise->setEnabled(
        src && this->ui->view->getCurrentPage() > 0);
    this->ui->actionRotate_180_degrees->setEnabled(
        src && this->ui->view->getCurrentPage() > 0);
    this->ui->actionSmart_scroll->setChecked(
        this->ui->view->smartScrollEnabled());
    this->ui->actionAllow_free_drag->setChecked(
        this->ui->view->freeDragAllowed());
    this->ui->actionMagnifying_lens->setChecked(
        this->ui->view->magnifyingLensEnabled());
}

void MainWindow::on_actionAdd_bookmark_triggered()
{
    int page = this->ui->view->getCurrentPage();
