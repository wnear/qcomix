    ...
    : QWidget(parent)
{
    ...
    connect(&this->slideShowTimer, &QTimer::timeout,
        [this]() { scrollNext(ScrollSource::SlideShowScroll); });
...
void PageViewWidget::initialize()
{
    ...
    for (int i = 0; i < CHECKERED_IMAGE_SIZE / CHECKER_SIZE; i++) {
        for (int j = 0; j < CHECKERED_IMAGE_SIZE / CHECKER_SIZE; j++) {
            if (i % 2 != j % 2) {
                painter.fillRect(i * CHECKER_SIZE, j * CHECKER_SIZE, CHECKER_SIZE,
                    CHECKER_SIZE, QColor(204, 204, 204));
            }
        }
    }
    painter.end();
...
bool PageViewWidget::onFirstPage() const { return !comic || currentPage < 2; }
...
bool PageViewWidget::verticallyFlipped() const { return this->verticalFlip; }

bool PageViewWidget::magnifyingLensEnabled() const { return this->magnify; }
...
void PageViewWidget::currentPageToClipboard()
{
    QApplication::clipboard()->setPixmap(
        QPixmap::fromImage(this->cachedPageImage));
}
...
bool PageViewWidget::getDoublePageMode() const { return this->doublePageMode; }

bool PageViewWidget::getMangaMode() const { return this->mangaMode; }
...
bool PageViewWidget::smartScrollEnabled() const { return this->smartScroll; }
...
bool PageViewWidget::freeDragAllowed() const { return this->allowFreeDrag; }

int PageViewWidget::slideShowInterval() const { return slideShowSeconds; }

ComicSource* PageViewWidget::comicSource() { return this->comic; }

int PageViewWidget::getCurrentPage() const { return currentPage; }
    ...
    if (active) {
        if (!keepTransformationOnPageSwitch || force) {
            ...
            if (fitMode == FitMode::ManualZoom) {
                setFitMode(
                    stringToFitMode(MainWindow::getOption("fitMode").toString()));
                emit this->fitModeChanged(fitMode);
            }
        }
        this->currentX = 0;
    ...
    if (active) {
        ...
        if (comic && currentPage > 0) {
            ...
            if (cachedPageImage.isNull()) {
                ...
                px_right = px_right.transformed(
                    transform, hqTransformMode ? Qt::SmoothTransformation : Qt::FastTransformation);
                ...
                QSize combined_size = QSize(px_left.width() + px_right.width(),
                    qMax(px_left.height(), px_right.height()));
                ...
                combined_painter.drawPixmap(
                    0, (px_combined.height() - px_left.height()) / 2, px_left);
                if (!px_right.isNull())
                    combined_painter.drawPixmap(
                        px_left.width(), (px_combined.height() - px_right.height()) / 2,
                        px_right);
            ...
            if (fitMode == FitMode::FitHeight) {
                if (px_combined.height() > height || stretchSmallImages) {
                    px_combined = px_combined.scaledToHeight(
                        height, hqTransformMode ? Qt::SmoothTransformation : Qt::FastTransformation);
                }
                cachedZoomBaseImageSize = px_combined.size();
            } else if (fitMode == FitMode::FitWidth) {
                if (px_combined.width() > width || stretchSmallImages) {
                    px_combined = px_combined.scaledToWidth(
                        width, hqTransformMode ? Qt::SmoothTransformation : Qt::FastTransformation);
                }
                cachedZoomBaseImageSize = px_combined.size();
            } else if (fitMode == FitMode::FitBest) {
                if (!(px_combined.width() < width && px_combined.height() < height) || stretchSmallImages) {
                    px_combined = px_combined.scaled(width, height, Qt::KeepAspectRatio,
                ...
                if (!(px_combined.width() < fixedSizeWidth && px_combined.height() < fixedSizeHeight) || stretchSmallImages) {
                    px_combined = px_combined.scaled(
                        fixedSizeWidth, fixedSizeHeight, Qt::KeepAspectRatio,
                        hqTransformMode ? Qt::SmoothTransformation
                                        : Qt::FastTransformation);
                }
                cachedZoomBaseImageSize = px_combined.size();
                ...
                if (!cachedZoomBaseImageSize.isNull())
                    zoomBaseImageSize = cachedZoomBaseImageSize;

                if (!cachedZoomedImage.isNull()) {
                    px_combined = cachedZoomedImage;
                } else {
                    px_combined = px_combined.scaled(
                        zoomScaleFactor * zoomBaseImageSize.width(),
                        zoomScaleFactor * zoomBaseImageSize.height(), Qt::KeepAspectRatio,
                        hqTransformMode ? Qt::SmoothTransformation
                                        : Qt::FastTransformation);
            ...
            emit this->updateHorizontalScrollBar(
                allowedXDisplacement, currentX, std::min(width, px_combined.width()));
            emit this->updateVerticalScrollBar(
                allowedYDisplacement, currentY,
                std::min(height, px_combined.height()));
            ...
            if (magnify && mouseCurrentlyOverWidget) {
                ...
                QImage sourceImg = px_combined
                                       .copy(mousePos.x() - targetX + currentX - sourceLength / 2.0,
                                           mousePos.y() - targetY + currentY - sourceLength / 2.0,
                                           sourceLength, sourceLength)
                                       .scaled(magnifyingLensSize, magnifyingLensSize,
                                           Qt::IgnoreAspectRatio,
                                           magnifyingLensHQScaling ? Qt::SmoothTransformation
                                                                   : Qt::FastTransformation);
                painter.fillRect(mousePos.x() - magnifyingLensSize / 2.0,
                    mousePos.y() - magnifyingLensSize / 2.0,
                    magnifyingLensSize, magnifyingLensSize, finalBkgColor);
                painter.drawImage(mousePos.x() - magnifyingLensSize / 2.0,
                    mousePos.y() - magnifyingLensSize / 2.0, sourceImg);
                painter.setPen(Qt::black);
                painter.drawRect(mousePos.x() - magnifyingLensSize / 2.0,
                    mousePos.y() - magnifyingLensSize / 2.0,
                    magnifyingLensSize, magnifyingLensSize);
            }
            qDebug() << zoomLevel;
        }
    } else {
...
void PageViewWidget::mouseMoveEvent(QMouseEvent* event)
{
    if (dragging) {
        ...
        ensureDisplacementWithinAllowedBounds();
        if (draggingXEnabled || draggingYEnabled || allowFreeDrag)
            update();
    }
    if (magnify) {
        mousePos = event->pos();
    ...
    PageViewWidget::ScrollSource src)
{
    ...
    if (direction == ScrollDirection::Down && currentY <= allowedYDisplacement) {
        ...
        if (mangaMode && currentY >= allowedYDisplacement && currentX <= 0) {
            currentFlipSteps++;
            if (src == ScrollSource::SlideShowScroll)
                currentFlipSteps = stepsBeforePageFlip;
        ...
        currentY += scrollPx;
        if (currentY > allowedYDisplacement)
            currentY = allowedYDisplacement;
        ...
        currentX += scrollPx;
        if (currentX > allowedXDisplacement)
            currentX = allowedXDisplacement;
    ...
    if (comic && comic->getPageCount() > 0 && currentPage > 0) {
        ...
        if (doublePageMode && currentPage < comic->getPageCount()) {
            if (!currentPageIsSinglePageInDoublePageMode())
                metadata2 = comic->getPageMetadata(currentPage);
        }
    }
...
void PageViewWidget::setCurrentPageInternal(int page)
{
    currentPage = page;
    if (comic) {
        emit this->currentPageChanged(comic->getFilePath(), currentPage,
            comic->getPageCount());
    } else {
...
QImage PageViewWidget::getCheckeredBackground(const QSize& size)
{
    ...
    for (int i = 0; i < size.width(); i += CHECKERED_IMAGE_SIZE) {
        for (int j = 0; j < size.height(); j += CHECKERED_IMAGE_SIZE) {
            painter.drawImage(i * CHECKERED_IMAGE_SIZE, j * CHECKERED_IMAGE_SIZE,
                checkeredBkg);
        }
    }
    return res;
